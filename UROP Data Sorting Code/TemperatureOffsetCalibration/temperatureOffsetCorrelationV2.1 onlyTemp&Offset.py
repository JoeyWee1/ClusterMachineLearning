...
def loadOrbitInfo(numbersLoc,startTimesLoc,endTimesLoc): 
    ...
    return orbitNumbers, orbitStartTimes, orbitEndTimes;

def loadTemperatureInfo(temperaturesLoc):
    ...
    return temperatureOrbitAverages;
...
def addTemperatures(temperatureInfo): #Adds temperature data to each orbit
    ...
    for i in range (0, len(temperatureOrbitAverages)):
        orbits[i].setTemperature(temperatureOrbitAverages[i]);
...
createOrbits(loadOrbitInfo("./orbit_numbers.npy","./orbit_start_times.npy","./orbit_end_times.npy")); #To loop through four spacecraft, just rewrite loadFileInfo to take in the file names as params then loop through array
addTemperatures(loadTemperatureInfo("./temperature_statistics.npy"));
