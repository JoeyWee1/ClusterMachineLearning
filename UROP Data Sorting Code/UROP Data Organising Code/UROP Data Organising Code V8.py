    ...
    for j in range(0, len(onPeriods[satelliteIndex])): #Loop through each onPeriod
        ...
        for k in range(0, len(orbits[satelliteIndex])): #Loops through each orbit to check if they should be added to the current onPeriod
            ...
            if ((orbits[satelliteIndex][k].startTime < endTime) and (orbits[satelliteIndex][k].startTime > startTime)):#Checks if the orbitStartTime is after the period start time and before the period end time
                ...
                print("orbit added")
                #print(tempPeriodOrbits)
        #onPeriods[satelliteIndex][j].addPeriodOrbits(tempPeriodOrbits);
...
addTemperatures(loadTemperatureInfo("./C1_temperature_statistics.npy",1));
##addTemperatures(loadTemperatureInfo("./C2_temperature_statistics.npy",2));
##addTemperatures(loadTemperatureInfo("./C3_temperature_statistics.npy",3));
##addTemperatures(loadTemperatureInfo("./C4_temperature_statistics.npy",4));
...
#%%TEST SECTION
createPeriods(loadPeriodInfo("./C1_power_on_times.txt",1));
#print(len(onPeriods[0]))
#print(len(onPeriods[1]))
#print(len(onPeriods[2]))
#print(len(onPeriods[3]))
#print(len(onPeriods))
#%%TEST SECTION
testSum = 0
for i in range(0,len(onPeriods[0])):   
    testSum += len(onPeriods[0][i].periodOrbits)
print(testSum)
