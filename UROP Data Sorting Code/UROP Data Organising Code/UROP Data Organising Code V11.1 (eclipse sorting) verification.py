...
class Orbit: #Creating orbit class
    ...
    def setTemperature(self,temperature, sigma): #sets the temperature; done like this so that more params can be added later
...
def loadTemperatureInfo(temperaturesLoc,satelliteNum): #Loads the temperature information for one satellite
    ...
    temperatureOrbitSigma = temperatureInformation[2][:]
    return temperatureOrbitAverages, satelliteNum-1, temperatureOrbitSigma; #Returns the average temperatures for each orbit consecutively; satellite num -1 to get index
...
def addTemperatures(temperatureInfo): #Adds temperature data to each orbit for one satellite as specified by the temperature info from loadTemperatureInfo function, which is the only param
    ...
    temperatureOrbitSigmas = temperatureInfo[2]
    for i in range (0, len(temperatureOrbitAverages)): #Loops through each orbit average temperature; the length is the same as the number of orbits as they match sequentially
        orbits[satelliteNumIndex][i].setTemperature(temperatureOrbitAverages[i],temperatureOrbitSigmas[i]); #For the correct satellite, each orbit recorded has the orbit average temperature added to it
    ...
    for i in range(0,4): #Loops through the 4 spacecraft
        for j in range(0,len(orbits[i])): #Loops through every orbit for that spacecraft
            ...
            if(orbitCleanliness == True):#Once all the eclipses have been searched, if there hasn't been an eclipse overlap found for this orbit, it is added to the cleaned array
                ...
                print(currentOrbit.sigma)
            else: #Else for exceptions
                print("Orbit {} for satellite {} excluded; sigma: {}".format(j+1,i+1,currentOrbit.sigma)); #Output
...
#%% TEST
print(len(cleanedOrbits[0]))
print(len(cleanedOrbits[1]))
print(len(cleanedOrbits[2]))
print(len(cleanedOrbits[3]))
