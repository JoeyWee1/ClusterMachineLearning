...
class OnPeriod:
    def __init__(self,startOrbit, endOrbit): #depends if start and end periods are defined by orbit or by time
        self.startOrbit = startOrbit; #Defining the start and ends so that the relevant orbits can be searched for
        self.endOrbit = endOrbit;
    def addPeriodOrbits(self,periodOrbits):
