...
#%% IMPORTING THE ORBITS AND DECLARING GLOBAL VARIABLES

onPeriods = np.load("./Inputs/sortedCleanedOrbitsArrayV3(Range2DataOnly).npy", allow_pickle = True);
...
def createAndTrainModel(splitData, fitDegree): #The splitData argument is the output of the createAndSplitData function
    ...
    model = SVR(kernel="poly", degree=fitDegree, gamma="scale"); #Creating the model
    model.fit(xTrain,yTrain); #Training the model on the data
    print("Model created")
    return model; #The model is an object and can be returned

#%% PLOT DATA AGAINST TIME WITH THE PREDICTIONS OVERLAID AND ONPERIOD RED LINES
...
def plotDataAndPredictionsVTime(splitData,model):
    ...
    #Plot the data
    saveTitle = "Test Period Data and Predictions for C{} {}-axis; Training Years {}; Degree {}".format(sc, axis, trainingYears, fitDegree);
    ...
    plt.savefig("./Outputs/{}/{}.png".format(fitDegree,saveTitle));
    
    
    
#%% TEST
for fitDegree in (1,2,3,4,5):
    for scIndex in range(0,4):
        for axisIndex in (1,2):
            splitData = createAndSplitData(onPeriods, scIndex, 18, axisIndex, ["F055", "F074","F047","F048", "F034"]);
            model = createAndTrainModel(splitData, fitDegree);
