...
def plotDataAndPredictionsVTime(splitData,model):
    width = 0.5;
    ...
    #Gets the temperatures into an array for CMap
    xTest = np.array(xTest);
    telemForColour = xTest[:,0];
    #Plot the data
    saveTitle = "Test Period Data and Predictions for C{} {}-axis; Training Years {}".format(sc, axis, trainingYears);
    title = "Test Period Data and Predictions for C{} {}-axis; Training Years {}".format(sc, axis, trainingYears);
    plt.rcParams["figure.dpi"] = 200;
    plt.suptitle(title);
    plt.xlabel("Test Set Data Split");
    plt.ylabel("Offset");
    plt.scatter(timeTest, yTest, marker = "x", s = 10, c = telemForColour, cmap='plasma', vmin=-20, vmax=10 , label = "Reference");
    plt.colorbar()
    plt.scatter(timeTest, yPred, marker = "o", s= 5, color = "red", label = "Predicted");
    plt.xlim(timeTest[0],timeTest[-1])
    plt.axvline(onTimes[0], color = "black",  linewidth=width, linestyle='--', label = "Power On/Off");
    plt.axvline(offTimes[0], color = "black", linewidth=width, linestyle= "--");
    for i in range(1,len(onTimes)):
        plt.axvline(onTimes[i], color = "black",  linewidth=width, linestyle='--');
        plt.axvline(offTimes[i], color = "black", linewidth=width, linestyle= "--");
    plt.scatter(timeTest[0],yTest[0], color = "white", label = "Error: {:.3f}".format(error));
    plt.scatter(timeTest[0],yPred[0], color = "white", label = "{}".format(features));
    ...
    plt.show();
    #plt.savefig("./Outputs RBF/{}.png".format(saveTitle));
...
for scIndex in range(0,4):
    for axisIndex in (1,2):
        splitData = createAndSplitData(onPeriods, scIndex, 15, axisIndex, ["F074","F055"]);
        model = createAndTrainModel(splitData, c, gamma);
        plotDataAndPredictionsVTime(splitData, model);
        del splitData, model
