...
def createAndSplitData(orbits, sc, trainingYears, axis): #Axis y = 1, z = 2
    ...
    for orbit in orbits[sc]: #Loop through every orbit for each spacecraft
        ...
        #featureVector = [orbit.F074, orbit.F055, orbit.F047,orbit.F034,orbit.F048]; #For each orbit, create a feature vector composed of the spacecraft/instrument telems
        #featureVector = [orbit.F074, orbit.F055, orbit.F047];
        featureVector = [orbit.F074, orbit.F055];
    ...
    return xTrain, xTest, yTrain, yTest, timeTrain, timeTest, sc, axis, trainingYears,splitTime;

#%% CREATE AND TRAIN MODEL ON TRAINING DATASET
def createAndTrainModel(splitData, fitDegree, fitKernal): #The splitData argument is the output of the createAndSplitData function
    ...
    timeTrain =  splitData[4]; #Times of the training data points
    trainingYears = splitData[8];
    totalTrainingTime = trainingYears * 31557600;
    firstOrbitStartTime = datetime.datetime(2000, 8, 24, 8, 56, 52);#First orbit start time
    #Use time train to form weights
    weights = [];
    for i in range(0,len(timeTrain)):
        timeSinceStart =  timeTrain[i] - firstOrbitStartTime;
        secondsSinceStart = timeSinceStart.total_seconds();
        weight = secondsSinceStart/totalTrainingTime; #Getting that as a fraction of the total time
        weights.append(weight);
    model = SVR(kernel=fitKernal, degree=fitDegree, C=100, epsilon=0.1, gamma="scale"); #Creating the model
    model.fit(xTrain,yTrain, sample_weight=weights); #Training the model on the data
...
def plotRegression(splitData, model): #Plots the regression in 3D space
    ...
    title = "Weighted SVM C{}; Training Years {}; Kernal {}; Degree {}; MeanAbsError = {:.2f}".format(sc+1,trainingYears,fitKernal,fitDegree,error);
    print(title)
    ax.set_title(title);
...
def plotErrorAgainstTime(testModelData):
    ...
    plt.xlabel("Years Since Training Period");
...
for scIndex in range(0,4):
    splitData = createAndSplitData(orbits, scIndex, trainingYears, 1);
    #splitTime = splitData[-1];
    meanAbsErrors = [];
    for degree in degrees:
        print("Cluster {}; Degree {}".format(scIndex+1,degree))
        model = createAndTrainModel(splitData, degree, fitKernal);
        meanAbsError = testModel(splitData, model)[5];
        meanAbsErrors.append(meanAbsError);
        #plotErrorAgainstTime(testModel(splitData,model));
    #errorsVDegrees[scIndex].append(degrees);
    errorsVDegrees[scIndex].append(meanAbsErrors);

for i in range (0,len(degrees)):
    sumError = errorsVDegrees[0][0][i] + errorsVDegrees[1][0][i] + errorsVDegrees[2][0][i] + errorsVDegrees[3][0][i];
    averageError = sumError/4;
    averageErrors.append(averageError);

#%%
fig, ax =plt.subplots(1,1, dpi=200);
plt.ylabel("Mean Absolute Error");
plt.xlabel("Poly Kernal Degrees");
title = "[F55,F74] Weighted Poly SVM Mean Absolute Errors Vs Kernal Degrees; Training Years: {}".format(trainingYears);
ax.set_title(title);
ax.plot(degrees, errorsVDegrees[0][0], label = "C1", color ="red");
ax.plot(degrees, errorsVDegrees[1][0], label = "C2", color ="blue");
ax.plot(degrees, errorsVDegrees[2][0], label = "C3", color ="lime");
ax.plot(degrees, errorsVDegrees[3][0], label = "C4", color ="yellow"); 
ax.plot(degrees, averageErrors, label = "Average", color ="fuchsia"); 
plt.legend();
plt.show();
