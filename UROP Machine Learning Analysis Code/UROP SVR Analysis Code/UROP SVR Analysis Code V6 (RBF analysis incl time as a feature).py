...
def createAndSplitData(orbits, sc, trainingYears, axis): #Axis y = 1, z = 2
    ...
    for orbit in orbits[sc]: #Loop through every orbit for each spacecraft
        ...
        timeSinceSplit = orbit.startTime - splitTime;
        secondsSinceSplit = timeSinceSplit.total_seconds();
        ...
        featureVector = [orbit.F074, orbit.F055,yearsSinceSplit]; #For each orbit, create a feature vector composed of the spacecraft/instrument telems
...
def createAndTrainRBFModel(splitData, c, gamma): #The splitData argument is the output of the createAndSplitData function 
    ...
    return model, c, gamma; #The model is an object and can be returned

#%% TEST MODEL ON TESTING DATASET
def testModel(splitData,modelData): #This function should return two arrays: absolute error of a data point, time since splitTime
...
def plotRegression(splitData, modelData): #Plots the regression in 3D space
    ...
    title = "RBF SVM C{}; Training Years {}; C: {}; Gamma {}; MeanAbsError = {:.2f}".format(sc+1,trainingYears,c,gamma,error);
    print(title)
    ax.set_title(title);
