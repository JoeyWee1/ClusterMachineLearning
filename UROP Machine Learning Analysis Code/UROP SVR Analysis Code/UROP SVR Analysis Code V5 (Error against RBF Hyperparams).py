...
for c in range (1,10,1):
    for gammax10 in range (1,100, 10):
        gamma = gammax10 / 10;
        print("Testing: C"+str(c)+"G"+str(gamma));
        splitData = createAndSplitData(orbits, 3, trainingYears, 2);
...
#%% PLOT DATA AS CMAP 
plt.scatter(cValues, gammaValues, c=errorValues, cmap='jet',vmin=0.01, vmax=0.1, );
plt.xlabel("C");
plt.ylabel("Gamma")
plt.colorbar()
plt.show()
