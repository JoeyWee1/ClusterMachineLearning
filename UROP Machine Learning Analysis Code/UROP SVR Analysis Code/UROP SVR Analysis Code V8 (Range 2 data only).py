...
def plotRegression(splitData, model): #Plots the regression in 3D space
    ...
    title = "SVM C{}; Training Years {}; Kernal {}; Degree {}; MeanAbsError = {:.2f}".format(sc+1,trainingYears,fitKernal,fitDegree,error);
    print(title)
    ax.set_title(title);
...
for scIndex in range(0,4):
    splitData = createAndSplitData(orbits, scIndex, trainingYears, 2);
    meanAbsErrors = [];
...
title = "Mean Absolute Errors Vs Kernal Degrees; Training Years: {}".format(trainingYears);
ax.set_title(title);
