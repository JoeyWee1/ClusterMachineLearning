...
for orbit in orbits[sc]: #Loop through every orbit for each spacecraft
        ...
        y = orbit.calParams[axis][0][0]; #Gets the z axis range 2 offset
...
k = 7; #Picking the number of nearest neighbours to consider
...
#%% PLOT DATA AND MODEL
#def getFeature(array, featureNumber):
    #feature = [];
    #for i in range(0,len(array)):
        #feature.append(array[i][featureNumber]);
    #return feature;

#xTestF0 = getFeature(xTest,0);
#xTestF1 = getFeature(xTest,1);
#xTrainF0 = getFeature(xTrain,0);
#xTrainF1 = getFeature(xTrain,1);

fig = plt.figure()
plt.rcParams['font.family'] = 'sans serif'
plt.rcParams["figure.dpi"] = 500
ax = fig.add_subplot(111, projection='3d')
#ax.scatter(xTrain[:, 0], xTrain[:, 1],yTrain, s = 5)
ax.scatter(xTest[:, 0],  xTest[:, 1],yTest , color = 'red', s = 0.5, label = "Reference")
ax.scatter(xTest[:, 0],  xTest[:, 1],pred , color = 'blue', s = 0.5, label = "Predicted")
ax.scatter(xTest[0],yTest[0], color = "white", label = "Error: {:.3f}".format(error));
title = "KNN Model Cluster {} {} Axis".format(sc+1, axes[axis]);
ax.set_title(title)
ax.set_xlabel('F074')
