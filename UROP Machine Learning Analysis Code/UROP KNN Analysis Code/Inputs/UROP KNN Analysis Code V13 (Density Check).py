...
def createAndSplitData(orbits, scIndex):
    ...
    for orbit in orbits[sc]: #Loop through every orbit for each spacecraft
            ...
            if (dataCleanliness == True):
                ...
                pass;
    xTrain, xTest, yTrain, yTest = train_test_split(featureVectors,offsets,test_size = 0);
...
def trainAndPlotModel(x1,y1,splitData, k):
    xTrain = splitData[0];
    xTest = x1;
    yTrain = splitData[2];
    yTest = y1;
...
def predictOffset(spacecraftNumber,axis,maxRadius,minCount,k,inputF074,inputF055):
    splitData = createAndSplitData(orbits, spacecraftNumber-1);
    ...
    if (clean == True):
        ...
        predictedOffset = trainAndPlotModel(inputF074, inputF055, splitData, k);
