#This code learns a KNN model based on the first n years of data and then tests the accuracy of the model for subsequent periods
...
def createAndTrainModel(splitData, k): #The splitData argument is the output of the createAndSplitData function
    ...
    model = neighbors.KNeighborsRegressor(n_neighbors=k,weights=weights); #Creating the model
...
def plotErrorDistribution(testModelData):
    ...
    plt.annotate(f'Predictions: {total_count}', xy=(0.5, 0.95), xycoords='axes fraction', fontsize=12,horizontalalignment='center', verticalalignment='top');
    plt.legend();
    # Add labels and title
    plt.xlabel('Absolute Error');
    plt.ylabel('Fraction of Frequency');
    plt.title('Error Distribution For KNN for Cluster {} {} Axis; Training Time {} years'.format(sc, axes[axis], trainingTime));
    plt.show();
...
for scIndex in range(0,4):
    ...
    splitData = createAndSplitData(orbits, scIndex, trainingYears, axis);
