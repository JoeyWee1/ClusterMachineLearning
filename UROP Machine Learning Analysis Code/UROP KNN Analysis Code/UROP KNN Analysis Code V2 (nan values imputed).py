...
for sc in range(0,4): #Loop through the four spacecraft
    for orbit in orbits[sc]: #Loop through every orbit for each spacecraft
        ...
        for i in range (0, len(featureVector)): #Looping through the coordinates in the feature vector to check for nan values for imputation
            if (str(featureVector[i]) =="nan"): #Checking for the nan values
                featureVector[i] = 1e4;#Replacing the nan values with an out of range values
        if (str(y) == "nan"):#Checks the y values for nan
            y = 1e4;#Replaces it with an out of bounds value
...
weights = "uniform"; #Electing to weigh the neighbours by distance
