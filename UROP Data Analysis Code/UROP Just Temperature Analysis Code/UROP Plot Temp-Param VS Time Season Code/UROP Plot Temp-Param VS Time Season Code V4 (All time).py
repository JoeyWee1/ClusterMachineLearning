...
def createArrayToPlot(orbits, startDatetime, endDatetime, sc, rng, axis, param): #sc, rng, axis, param as desired; orbits is the 2d array of orbits; start and end datetimes must be input as datetime objects
    ...
    for i in range(0,len(orbitsInPeriod)):#Looping through the array of in-period orbits
        ...
        currentTelem = currentOrbit.F034; #Currently, this is temperature, but this is general and can be altered
...
def plotTelemParamVTime(arrayData): #Takes in the output array of the createArrayToPlot function to plot the temperatures and a param against time; can be altered once more telems are added
    ...
    title = "F074-{} VS Time {} Axis Range {} Cluster {}".format(param, axis, rng, sc); #Formats the title string
    ...
    ax1.scatter(datetimes, telemValues, marker = "x", color = "blue"); #Plot the telemetry values
    ax1.set_ylabel("F034"); #Set the label to temperature; the only telem rn is temperature so this is the format
    ax2.scatter(datetimes, paramValues, marker = "x", color = "blue"); #Plot the calibration parameter
...
#%% RUN EVERYTHING
startDatetime = datetime.datetime(2000, 4, 1, 0, 0, 0); #The datetime of the start of the relevant period
endDatetime =  datetime.datetime(2020, 1, 31, 0, 0, 0); #The datetime of the end of  the relevant period
...
axis = 2
param = 0          
plotTelemParamVTime(createArrayToPlot(orbits,startDatetime, endDatetime, sc, rng, axis, param));
