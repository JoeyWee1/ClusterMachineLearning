...
#%% A FUNCTION TO DO THE PLOTTING FOR ONE TEMP-PARAM PAIR VS TIME #Can be amended later to change the telem 

def plotTemperatureParamVTime(scIndex, axisIndex, paramIndex, rngIndex, onPeriod, threshold): #Plots one OnPeriod for a set of calibration data
    ...
    if(len(arrayToPlot) >= threshold):#We only want to plot the onPeriods with more than the threshold number of orbits
        ...
        fig, (ax1, ax2) = plt.subplots(2, 1, sharex=True, dpi = 50); #Creates the subplots
        title = "Temperature-{} VS Time {} Axis Range {} Cluster {} for On-Period between {} and {}".format(paramName, axisName, rangeName, spacecraftName, periodStartTime, periodEndTime); #Formats the title string
        ...
        for i in range(0, len(arrayToPlot)): #Looping through the triples to pull the data and append them to the relevant arrays
            ...
            print(arrayToPlot[i][0])
            paramValues.append(arrayToPlot[i][1]);
            print(arrayToPlot[i][1])
            datetimes.append(arrayToPlot[i][2]);
            print(arrayToPlot[i][2])
        ...
        ax2.set_ylabel(paramName); #Labelling the param
        fig.autofmt_xdate(); #This formats the x axis dates
        savetitle = "Temp-{} V Time for {} to {}".format(paramName, periodStartTime.date(), periodEndTime.date())
...
def runTemperatureParamVTime(): #Function to do the plotting for every onPeriod for every spacecraft
            ...
            for rng in range(0,6): #Loops through the 6 ranges
                for param in range(0,4): #Loops through the 4 parameters
                    for axis in range(0,3): #Loops through the 3 axes
                        plotTemperatureParamVTime(sc, axis, param, rng, onPeriod, threshold);
                        
#%% A FUNCTION TO DO THE PLOTTING FOR ONE TEMP-PARAM PAIR AGAINST EACH OTHER 
