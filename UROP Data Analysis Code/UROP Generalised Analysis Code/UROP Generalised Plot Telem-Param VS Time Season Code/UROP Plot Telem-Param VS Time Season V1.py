...
def createArrayToPlot(orbits, startDatetime, endDatetime, sc, rng, axis, param, telem): #sc, rng, axis, param as desired; orbits is the 2d array of orbits; start and end datetimes must be input as datetime objects
...
def plotTelemParamVTime(arrayData): #Takes in the output array of the createArrayToPlot function to plot the temperatures and a param against time; can be altered once more telems are added
    ...
    title = "{}-{} VS Time {} Axis Range {} Cluster {} {}'-{}' Season".format(telem, param, axis, rng, sc, startYear, endYear); #Formats the title string
    ...
    ax1.set_ylabel(telem); #Set the label to temperature; the only telem rn is temperature so this is the format
...
def plotTelemVParam(arrayData): #Takes in the output array of the createArrayToPlot function to plot the temperatures against a param; can be altered once more telems are added
    ...
    title = "{} VS {}  {} Axis Range {} Cluster {} {}'-{}' Season".format(telem, param, axis, rng, sc, startYear, endYear); #Formats the title string
    fig.suptitle(title); #Creates the title
    telemValues = []; #Creating arrays into which to put the data pulled from the triples
    paramValues = [];
    for i in range(0, len(data)): #Looping through the triples to pull the data and append them to the relevant arrays
...
#%% RUN EVERYTHING
for seasonStartYear in range(2000,2020):
    startDatetime = datetime.datetime(seasonStartYear, 4, 1, 0, 0, 0); #The datetime of the start of the relevant period
    endDatetime =  datetime.datetime(seasonStartYear+1, 1, 31, 0, 0, 0); #The datetime of the end of  the relevant period
    for sc in range(0,4): #Loops through the four spacecraft
        #for rng in range(0,6): #Loops through the 6 ranges
            ...
            for axis in range(1,3): #Loops through the 3 axes
                for telem in telems:
                    #plotTelemVParam(createArrayToPlot(uncleanedOrbits,startDatetime, endDatetime, sc, rng, axis, param, telem));
                    #plotTelemVParam(createArrayToPlot(uncleanedOrbits,startDatetime, endDatetime, 0, 0, 2, 0));
                    plotTelemParamVTime(createArrayToPlot(uncleanedOrbits,startDatetime, endDatetime, sc, rng, axis, param,telem));
